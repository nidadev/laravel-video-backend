...
$(document).ready(function() {
  ...
  $('#video-files-container').on('change','.image-file', async function(){
    ...
    $(this).data('s3-url', data.file_url);
    $(this).siblings('input[type="hidden"]').val(data.file_url);
    alert('Episode image uploaded!');
  });

  // Video file upload (new)
  $('#video-files-container').on('change','.video-file', async function(){
    const file = this.files[0]; if(!file) return;
    const res = await fetch('{{ route("admin.videos.presigned.url") }}',{
      method:'POST',
      headers:{ 'X-CSRF-TOKEN':'{{ csrf_token() }}' },
    ...
    $(this).data('s3-url', data.file_url);
    let hidden = $(this).siblings('.video-url');
    if(hidden.length===0) hidden = $('<input type="hidden" class="video-url">').insertAfter(this);
    hidden.val(data.file_url);
  ...
  $('#presignedEditForm').on('submit', async function(e){
    ...
    // Existing files
    $('#video-files-container .video-file-item').each(function(){
      const fileId = $(this).find('input[name*="existing_files"][name*="[id]"]').val();
      if(fileId){
        formData.append(`existing_files[${fileId}][id]`, fileId);
        formData.append(`existing_files[${fileId}][file_url]`, $(this).find('input[name*="[file_url]"]').val());
        formData.append(`existing_files[${fileId}][image]`, $(this).find('input[name*="[image]"]').val());
        formData.append(`existing_files[${fileId}][variant]`, $(this).find('.variant').val());
        formData.append(`existing_files[${fileId}][season_id]`, $(this).find('.season-select').val());
        formData.append(`existing_files[${fileId}][drm]`, $(this).find('.drm').val());
        formData.append(`existing_files[${fileId}][duration]`, $(this).find('.duration').val());
      }
    });

    // New videos
    $('#video-files-container .video-file-item').each(function(i){
      if(!$(this).find('input[name*="existing_files"]').length){
        const videoFile = $(this).find('.video-file')[0];
        const imgFile = $(this).find('.image-file')[0];
        formData.append(`new_videos[${i}]`, $(videoFile).data('s3-url') || '');
        formData.append(`new_images[${i}]`, $(imgFile).data('s3-url') || '');
        formData.append(`new_variants[${i}]`, $(this).find('.variant').val());
        formData.append(`new_seasons[${i}]`, $(this).find('.season-select').val());
        formData.append(`new_drms[${i}]`, $(this).find('.drm').val());
        formData.append(`new_durations[${i}]`, $(this).find('.duration').val());
      }
    });

    const res = await fetch("/admin/videos/{{ $video->id }}/update", {
    method: "POST",
    headers: { "X-CSRF-TOKEN": "{{ csrf_token() }}" },
    body: formData
});
